...
public class MyEmail
{
	...
	// public void send(String from, String[] to, String subject, String text,
	// String[] cc, String[] bcc, String[] attachFullFilenames)
	// {
	// try
	// {
	...
	//
	// MyException.throwIf(() -> from==null||from.length()==0,"from no puede ser
	// null ni vacío");
	// MyException.throwIf(() -> to == null || to.length == 0,"to no puede ser
	// null ni vacío");
	//
	// helper.setFrom(from);
	// helper.setTo(to);
	...
	// ByteArrayDataSource(Files.readAllBytes(path),Files.probeContentType(path));
	...
	//
	// ByteArrayDataSource ds = new
	// ByteArrayDataSource(Files.readAllBytes(path),contentType);
	//
	...
	{
		MyEmailAttachment[] atts=null;

		if(attachFullFilenames!=null)
		{
			atts=new MyEmailAttachment[attachFullFilenames.length];

			for(int i=0; i<attachFullFilenames.length; i++)
			{
				String filename=attachFullFilenames[i];
				if(filename==null) continue;

				atts[i]=new FileEmailAttachment(Paths.get(filename));
			}
		}

		send(from,to,subject,text,cc,bcc,atts);
	}

	public void send(String from, String[] to, String subject, String text, String[] cc, String[] bcc, MyEmailAttachment[] attachments)
	{
		try
		{
			MimeMessage mime=javaMailSender.createMimeMessage();
			...
			MyException.throwIf(() -> from==null||from.isBlank(),"from no puede ser null ni vacío");
			...
			if(attachments!=null)
			{
				for(MyEmailAttachment att:attachments)
				{
					if(att==null) continue;

					helper.addAttachment(att.getFileName(),att.getDataSource());
				}
			}
		...
		try
		{
			...
			for(; i<n; i++)
			{
				try
				{
					...
					MyEmailAttachment[] attach=ds.getAttachments(i);
